...
function App() {
  ...
  async function load() {
    ...
    try {
      ...
      setError('')
    } catch (loadError) { setError(loadError.message); setMessage('') } finally { setLoading(false) }
  }

  useEffect(() => {
    let active = true
    startTransition(() => setLoading(true))
    fetchTasks(search, filter).then((data) => { if (active) startTransition(() => { setTasks(data.tasks); setMessage(`${data.tasks.length} task${data.tasks.length === 1 ? '' : 's'} found`); setError('') }) }).catch(() => { if (active) { setError('Could not load tasks. Is the backend running?'); setMessage('') } }).finally(() => { if (active) setLoading(false) })
    return () => { active = false }
  }, [filter, search])
  ...
  async function saveTask(event) {
    ...
    try {
      ...
      if (!response.ok) { const data = await response.json(); throw new Error(data.error || 'The task could not be saved.') }
  ...
  async function syncNow() {
    ...
    try {
      ...
      await load()
    } catch (syncError) { setError(syncError.message); setMessage('Sync failed.') } finally { setSyncing(false) }
  }
  ...
  async function deleteTask(task) {
    ...
    setError('')
    try {
      const response = await fetch(`${API}/tasks/${task.id}`, { method: 'DELETE', headers: { 'If-Match': String(task.version) } })
      if (!response.ok) { const data = await response.json(); throw new Error(data.error || 'The task could not be deleted.') }
      await load()
    } catch (deleteError) { setError(deleteError.message) }
  }

  async function resolve(task, choice) {
    setResolving(`${task.id}:${choice}`); setError('')
    try {
      const response = await fetch(`${API}/tasks/${task.id}/resolve`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ choice }) })
      const data = await response.json()
      if (!response.ok) throw new Error(data.error || 'Conflict resolution failed.')
      setMessage(choice === 'local' ? 'Local version resolved and sent to GitHub.' : 'GitHub version resolved locally.')
      await load()
    } catch (resolveError) { setError(resolveError.message) } finally { setResolving(null) }
  }

  return <main className="shell">
    <header className="topbar"><div><p className="kicker">TASKS</p><h1>Tasks in one place.</h1><p className="lede">Create and update tasks here or in GitHub. This page shows which version is current.</p></div><button className="sync-button" onClick={syncNow} disabled={syncing || loading} title="Run bidirectional sync">{syncing ? 'Syncing...' : '↻ Sync now'}</button></header>
    <section className="metrics"><div><span>TASKS</span><strong>{tasks.length}</strong></div><div><span>NEEDS REVIEW</span><strong>{tasks.filter((task) => ['conflict', 'error'].includes(task.syncStatus)).length}</strong></div><div><span>STATUS</span><strong className="signal">{error || (syncing ? 'Syncing with GitHub...' : message)}</strong></div></section>
    <section className="workspace"><aside><div className="new-task"><p className="eyebrow">{editing ? 'EDIT TASK' : 'ADD A TASK'}</p><form onSubmit={saveTask}><input aria-label="Task title" required placeholder="Task title" value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} /><textarea aria-label="Task description" placeholder="Description (optional)" value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} /><button className="primary" type="submit" disabled={saving}>{saving ? 'Saving...' : editing ? 'Save changes' : 'Add task'}</button>{editing && <button className="quiet" type="button" onClick={() => { setEditing(null); setDraft({ title: '', description: '' }) }}>Cancel</button>}</form></div><div className="policy"><span className="dot amber" />When there is a conflict<strong>You choose the version</strong><p>Local changes stay here until you decide whether to keep them or use the GitHub version.</p></div></aside>
      <div className="list"><div className="list-head"><div><p className="eyebrow">TASK LIST</p><h2>All tasks</h2></div><input aria-label="Search tasks" className="search" placeholder="Search" value={search} onChange={(event) => setSearch(event.target.value)} /></div><nav className="filters">{['all', 'synced', 'pending', 'conflict', 'error'].map((item) => <button className={filter === item ? 'selected' : ''} key={item} onClick={() => setFilter(item)}>{item}</button>)}</nav><div className="task-list">{loading ? <div className="empty">Loading tasks...</div> : tasks.length === 0 ? <div className="empty">No tasks found. Add a task or change the filter.</div> : tasks.map((task) => <article className={`task ${task.syncStatus}`} key={task.id}><div className="task-main"><div className="task-title"><span className={`status ${task.syncStatus}`} /> <h3>{task.title}</h3></div><p>{task.description || 'No description added.'}</p>{task.providerUrl && <a href={task.providerUrl} target="_blank" rel="noreferrer">View on GitHub ↗</a>}{task.conflict && <div className="versions"><div><strong>This app</strong><span>{task.conflict.local.title}</span><small>{task.conflict.local.description || 'No description'}</small></div><div><strong>GitHub</strong><span>{task.conflict.remote.title}</span><small>{task.conflict.remote.description || 'No description'}</small></div></div>}</div><div className="task-meta"><span className={`badge ${task.syncStatus}`}>{task.syncStatus}</span>{task.error && <small>{task.error}</small>}{task.syncStatus === 'conflict' ? <div className="conflict-actions"><button disabled={Boolean(resolving)} onClick={() => resolve(task, 'local')}>{resolving === `${task.id}:local` ? 'Resolving...' : 'Keep this version'}</button><button disabled={Boolean(resolving)} onClick={() => resolve(task, 'remote')}>{resolving === `${task.id}:remote` ? 'Resolving...' : 'Use GitHub version'}</button></div> : <div><button className="edit" onClick={() => { setEditing(task); setDraft({ title: task.title, description: task.description }) }}>Edit</button><button className="delete" onClick={() => deleteTask(task)}>Delete</button></div>}</div></article>)}</div></div>
    </section>
  </main>
}
