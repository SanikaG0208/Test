...
You need Node 18 or newer, PostgreSQL 14 or newer, and a GitHub token that can read and write Issues in the repository you want to use.
...
The PostgreSQL implementation currently stores the sync state as one JSON document. A production version serving multiple backend instances should normalize tasks and webhook events into tables with row-level leases and transactional outbox processing. The current pull uses GitHub's `since` filter but still needs to scan each changed page; using GitHub events or conditional requests would make large repositories more efficient.
